 ...
 * inline `<style>`, a `<script>`, an event handler attribute, an `<svg>` subtree
 * and the document an `<iframe srcdoc>` holds. A `style=""` is not among them:
 * how the attribute is written is decided by reading the minified text back, so
 * it stays with the built-in CSS minifier, which is webpack's own `cssMinify`.
 * @returns {string[]} the languages
 */

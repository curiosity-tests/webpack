 ...
 * JavaScript, every event handler attribute that holds a whole script, an
 * `<svg>` subtree, and the document an `<iframe srcdoc>` holds. Returning the input unchanged declines
...
// Whether each handler body seen in this print is a whole script, keyed by the
// text: one document's handlers repeat, and every miss costs a parse.
/** @type {Map<string, boolean>} */
const _wholeScriptCache = new Map();

// acorn, for the one question this printer asks about JavaScript. Required on
// the first handler offered rather than at module scope, as the CSS minifier is.
/** @type {typeof import("acorn") | undefined} */
let _acorn;

/**
 * Whether this text is a whole script, which is the production a renderer is
 * offered. An event handler attribute holds a function body, so one whose top
 * level is a `return` is left alone — a minifier of scripts handed it would
 * report the parse error rather than answer.
 * @param {string} source the handler's text
 * @returns {boolean} true when it parses as a script
 */
const _isWholeScript = (source) => {
	const memoized = _wholeScriptCache.get(source);
	if (memoized !== undefined) return memoized;
	let parses = true;
	try {
		const acorn = _acorn || (_acorn = require("acorn"));
		acorn.parse(source, { ecmaVersion: "latest", sourceType: "script" });
	} catch (_err) {
		parses = false;
	}
	_wholeScriptCache.set(source, parses);
	return parses;
};
	...
	for (let k = 0; k < count; k++) {
		...
		if (quoted) {
			...
			if (
				...
				if (_deferEmbeddedSource !== undefined) {
					out += _deferEmbedded(handler, JAVASCRIPT_TYPE, (rendered) => {
					...
					const answered = writeHandler(
						_renderEmbeddedOrNull(handler, JAVASCRIPT_TYPE)
					);
